...
const itemTaxRate    = (li) => li.taxExempt ? 0 : TAX_RATE;
...
function OrderForm({ initial, onSave, onCancel }) {
  ...
  const fakeOrder = { lineItems, payments, discount, discountType, taxExempt };
  const subtotal  = orderSubtotal(fakeOrder);
  const discAmt   = orderDiscount(fakeOrder);
  const afterDisc = orderAfterDisc(fakeOrder);
  const tax       = orderTax(fakeOrder);
  const total     = orderTotal(fakeOrder);
