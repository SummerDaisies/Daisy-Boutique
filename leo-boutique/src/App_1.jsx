    ...
    <div style={{ background:"#0a1520", border:`1px solid ${missingStatus?"#fb718544":sc+"22"}`, borderLeft:`3px solid ${missingStatus?"#fb7185":sc}`, borderRadius:10, padding:14, display:"flex", flexDirection:"column", gap:10 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          ...
          {itemTotal>0 && (
            <span style={{ fontSize:11, color:"#c9a96e", background:"#c9a96e18", border:"1px solid #c9a96e33", borderRadius:20, padding:"2px 10px", fontWeight:700 }}>
              {fmt(itemTotal)} + tax = {fmt(itemTotalWithTax)}
            </span>
          )}
        </div>
      ...
      <div style={{ background:"#070d14", border:"1px solid #818cf822", borderRadius:8, padding:10 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
          <div><label style={LS}>Amount Paid on This Item ($)</label>
            <input style={{ ...IS, border:"1px solid #818cf822" }} type="number" step="0.01"
              value={li.itemPaid||""} onChange={e=>inp("itemPaid",e.target.value)} placeholder="0.00" />
          </div>
          <div style={{ background:"#0a1520", border:"1px solid #818cf822", borderRadius:6, padding:"8px 10px", display:"flex", flexDirection:"column", gap:3 }}>
            {(()=>{
              ...
              const balance = itemTotalWithTax - totalPaidOnItem;
              return (<>
                <div style={{ fontSize:10, color:"#64748b" }}>Total w/Tax: <span style={{ color:"#c9a96e", fontWeight:700 }}>{fmt(itemTotalWithTax)}</span></div>
                <div style={{ fontSize:10, color:"#64748b" }}>Total Paid: <span style={{ color:"#4ade80", fontWeight:700 }}>{fmt(totalPaidOnItem)}</span></div>
                <div style={{ fontSize:11, fontWeight:800, color:balance>0?"#fb7185":"#4ade80" }}>Balance: {fmt(Math.max(0,balance))}{balance<=0?" ✓":""}</div>
              </>);
            })()}
          </div>
        </div>
      ...
      <div style={{ display:"flex", gap:10, justifyContent:"flex-end" }}>
        ...
        <button onClick={()=>{
          ...
          onSave({
            ...
            modelNumber, colorPref, eventDate, photoDate,
